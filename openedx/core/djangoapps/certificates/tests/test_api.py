...
ALLOWLISTED_METHOD = 'openedx.core.djangoapps.certificates.api.certs_api.is_on_allowlist'
...
class CertificatesMessagingTestCase(ModuleStoreTestCase):
    ...
    def test_beta_tester(self):
        grade = CourseGradeFactory().read(self.user, self.course)
        certs_enabled = True

        with patch(ALLOWLISTED_METHOD, return_value=True):
